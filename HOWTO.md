...
DISPATCH_OFFLINE_FIXTURE=true dispatch demo "Routing review" \
  --workflow-file examples/workflows/routed-review.json \
  --yes \
  --non-interactive \
  --output-root tests/tmp/routing-howto
...
Routing policy version `1.0.0` supports `quality_first`, `cost_first`, `latency_first`, and `balanced`. Hard constraints are identical for every objective; the objective changes only deterministic ranking among eligible candidates. Unsupported versions or objectives fail explicitly.
...
Use `handler_id: "model"` for an agent that does not need custom Kujo handler code. Supply `instructions`, `model_candidates`, optional registered `tools`, `model_options`, and an optional `output_schema`. Dispatch sends tool schemas to the model, executes only the declared tools the model actually requests, reapplies authorization on every call, deduplicates repeated tool-call IDs, and stops at the configured bounds (`max_tool_rounds` 0–16 and `max_tool_calls` 0–64).

To persist incremental AI SDK stream events while retaining the same normalized final result:
...
The run directory receives `stream-<step-id>.jsonl` as deltas arrive.
...
Live SDK calls are the default and fail closed when credentials or configuration are missing. Do not set fixture mode for a live run:
...
export DISPATCH_ALLOWED_CUSTOM_PROVIDER_ORIGINS=https://watchdog.example/v1
```
...
For an intentionally local OpenAI-compatible endpoint over HTTP, set `KUJO_AI_SDK_ALLOW_INSECURE_LOCALHOST=1`, `DISPATCH_ALLOW_INSECURE_LOCAL_CUSTOM_PROVIDER=true`, `KUJO_ALLOW_PRIVATE_NETWORK_DESTINATIONS=1`, and include the exact localhost base URL in `DISPATCH_ALLOWED_CUSTOM_PROVIDER_ORIGINS`. These opt-ins apply only to local development; production endpoints must use HTTPS.

## Bound Time, Tokens, Cost, and Work

Add workflow-level budgets. Dispatch persists cumulative use and fails with `workflow_budget_exceeded` before starting more work or immediately after a model response crosses a resource limit.
...
Concurrency is explicit and side-effect safe. Set a workflow bound and opt individual tool steps in with a stable idempotency key:
...
Only dependency-ready `tool` steps can opt in. Dispatch starts at most `max_parallel_steps`, preserves deterministic result merge order, and keeps agent/approval/handoff/report execution serialized.

## Configure Durable State and Delivery

Filesystem state is the zero-dependency default. SQLite is available for stronger restart and shared-root state authority:

```bash
export DISPATCH_STATE_BACKEND=sqlite
dispatch doctor --output-root outputs --json
```

SQLite uses WAL, full synchronous durability, and revision compare-and-swap. Human-readable JSON artifacts are still written as mirrors. Every run also has an owner-bound lock; tune waits only when deployment needs differ:
...
```bash
export DISPATCH_WEBHOOK_SIGNING_KEY='from-your-secret-manager'
export DISPATCH_WEBHOOK_SIGNING_KEY_ID='events-2026-08'
export DISPATCH_ALLOWED_WEBHOOK_ORIGINS='https://collector.example'
dispatch run "Task" --workflow-file workflow.json --webhook-url https://collector.example/events --yes --non-interactive
...
The outbox stores a destination hash rather than the URL. Replay therefore requires the operator to supply the same URL. The active outbox rotates to one bounded backup at `DISPATCH_WEBHOOK_OUTBOX_MAX_BYTES` (10 MiB by default).

Receivers verify `signature.value` as HMAC-SHA256 over the canonical JSON object containing `schema_version`, `event_id`, `issued_at`, `destination`, `url_hash`, and `payload`, using the key selected by `signature.key_id`. Reject stale `issued_at` values and duplicate `event_id` values.

## Check a Deployment

`doctor` always checks the selected state backend. `--live` additionally validates the workflow's environment and performs a small real request against every unique catalog route:

```bash
dispatch doctor --live --workflow-file workflow.json --output-root outputs --json
```

This command uses real provider credentials and may incur minimal provider usage. Do not run `--live` in fixture-only CI.
...
- `otel-traces.json`: OTLP JSON spans correlated by run/step/trace IDs;
- `stream-<step-id>.jsonl`: incremental model events when streaming is enabled;
...
Unset `DISPATCH_OFFLINE_FIXTURE` in the environment used by Dispatch and verify `AI_SDK_PATH`, `KUJO_BIN`, the provider key, and `DISPATCH_SDK_BRIDGE_SCRIPT` when overridden.
