...
The source-level upgrade rehearsal ran both versions under the current pinned
runtime, not the original 1.2 runtime. None of
...
| P1 | Stage a quiesced 1.2-to-1.3 upgrade/rollback with the actual prior and candidate runtimes and representative paused runs. The source-level rehearsal passed both CI platforms and state backends but used one current runtime. Never share a live output root between versions. | Target-staging backup/recovery, paused-run and separate-worker receipts on both state backends; source-level CI alone does not prove a production upgrade. |
