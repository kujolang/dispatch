...
- [ ] Quiesce pre-1.3 workers before replacing age-reclaimable lock files; validate resumed legacy runs and document rollback without mixing lock protocols. A local v1.2-source rehearsal passed both state backends under Kujo 1.4.0; CI and target staging with the original runtime remain pending.
- [ ] The installer handles both the legacy root bridge path exported by the pinned shim and the `src/bridge/` source layout without enabling arbitrary config paths. A real isolated pinned macOS installation at `1a82def` passed; verify final-revision Linux/macOS CI and authorized tag installs.
...
- [x] Linux and macOS pass the exact pinned full CI gate: SDK, routing, SQLite, policy, operational, hardening, 24 contract shards, warning-free VM/interpreter smoke, and bounded 3/3 workload. [CI run 35735638167](https://github.com/kujolang/dispatch/actions/runs/35735638167) passed both platforms at `1a82def`; repeat at the final revision.
